...
const totalAmtRaised = GAMES_JSON.reduce((sum, game) => {
    return sum + game['pledged']
}, 0);

// set inner HTML using template literal
raisedCard.innerHTML = `$${totalAmtRaised}`;
...
const unfundedGamesMsg = `
    A total of $${totalAmtRaised.toLocaleString('en-US')} has been raised for 4 games. Currently, 
    ${totalUnfundedGames > 1 ? `${totalUnfundedGames} games remain` : '1 game remains'} unfunded. 
    We need your help to fund these amazing games!
`;

// create a new DOM element containing the template string and append it to the description container
const unfundedMsg = document.createElement('p');
unfundedMsg.innerHTML = unfundedGamesMsg;
